...
gene_module_df_sig <- gene_module_df %>% filter(id %in% pr_deg_ids_1)
...
pheatmap::pheatmap(t(agg_mat), angle_col = 90, cutree_cols = 2, cutree_rows = 2, color = my_palette, breaks = breaks,
...
n_colors <- length(unique(plot_df$annotation))  # 또는 원하는 수
